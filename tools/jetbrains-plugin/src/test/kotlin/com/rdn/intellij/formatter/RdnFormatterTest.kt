...
class RdnFormatterTest {
    ...
    fun testPreservesInvalidInput() {
        ...
        assertEquals(invalid, format(invalid), "Invalid input should be returned unchanged")
    }

    @Test
    fun testFormatSortedReturnsNullForInvalid() {
        assertNull(formatSorted("not valid rdn"), "formatSorted should return null for invalid input")
    }

    @Test
    fun testTrailingNewline() {
        assertTrue(format("42").endsWith("\n"), "Formatted output must end with newline")
    }

    @Test
    fun testFormatSortedTrailingNewline() {
        assertTrue(formatSorted("{\"a\": 1}")?.endsWith("\n") ?: false, "formatSorted output must end with newline")
    }
