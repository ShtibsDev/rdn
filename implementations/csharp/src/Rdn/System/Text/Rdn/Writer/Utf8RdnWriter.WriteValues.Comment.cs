...
namespace Rdn
{
    public sealed partial class Utf8RdnWriter
    {
        ...
        /// <summary>
        /// Writes the string text value (as a RDN comment).
        /// </summary>
        /// <param name="value">The value to write as a RDN comment within /*..*/.</param>
        ...
        /// <summary>
        /// Writes the text value (as a RDN comment).
        /// </summary>
        /// <param name="value">The value to write as a RDN comment within /*..*/.</param>
        /// <exception cref="ArgumentException">
        ...
        public void WriteCommentValue(ReadOnlySpan<char> value)
        {
            ...
            if (_tokenType is RdnTokenType.PropertyName or RdnTokenType.None)
            {
        ...
        private void WriteCommentByOptions(ReadOnlySpan<char> value)
        {
            ...
            {
                // Comments generally can be placed anywhere in RDN, but not after a non-final
                // string segment.
        ...
        {
            Debug.Assert(value.Length < (int.MaxValue / RdnConstants.MaxExpansionFactorWhileTranscoding) - 4);

            // All ASCII, /*...*/ => escapedValue.Length + 4
            // Optionally, up to 3x growth when transcoding
            int maxRequired = (value.Length * RdnConstants.MaxExpansionFactorWhileTranscoding) + 4;
            ...
            output[BytesPending++] = RdnConstants.Asterisk;

            OperationStatus status = RdnWriterHelper.ToUtf8(value, output.Slice(BytesPending), out int written);
        ...
        private void WriteCommentIndented(ReadOnlySpan<char> value)
        {
            ...
            Debug.Assert(value.Length < (int.MaxValue / RdnConstants.MaxExpansionFactorWhileTranscoding) - indent - 4 - _newLineLength);

            // All ASCII, /*...*/ => escapedValue.Length + 4
            // Optionally, 1-2 bytes for new line, and up to 3x growth when transcoding
            int maxRequired = indent + (value.Length * RdnConstants.MaxExpansionFactorWhileTranscoding) + 4 + _newLineLength;
            ...
            if (_tokenType != RdnTokenType.None || _commentAfterNoneOrPropertyName)
            {
                WriteNewLine(output);
        ...
        /// <summary>
        /// Writes the UTF-8 text value (as a RDN comment).
        /// </summary>
        /// <param name="utf8Value">The UTF-8 encoded value to be written as a RDN comment within /*..*/.</param>
        /// <remarks>
        /// The comment value is not escaped before writing.
        /// </remarks>
        /// <exception cref="ArgumentException">
        ...
        public void WriteCommentValue(ReadOnlySpan<byte> utf8Value)
        {
            ...
            if (!RdnWriterHelper.IsValidUtf8String(utf8Value))
            {
