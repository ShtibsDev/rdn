...
namespace Rdn
{
    ...
    {
        internal bool TryGetNamedPropertyValue(int index, ReadOnlySpan<char> propertyName, out RdnElement value)
        {
            CheckNotDisposed();
            ...
            int maxBytes = RdnReaderHelper.s_utf8Encoding.GetMaxByteCount(propertyName.Length);
            int startIndex = index + DbRow.Size;
            int endIndex = checked(row.NumberOfRows * DbRow.Size + index);

            if (maxBytes < RdnConstants.StackallocByteThreshold)
            {
                Span<byte> utf8Name = stackalloc byte[RdnConstants.StackallocByteThreshold];
                int len = RdnReaderHelper.GetUtf8FromText(propertyName, utf8Name);
            ...
            while (candidateIndex > index)
            {
                ...
                if (row.SizeOrLength >= minBytes)
                {
                    ...
                    try
                    {
                        int len = RdnReaderHelper.GetUtf8FromText(propertyName, tmpUtf8);
        ...
        internal bool TryGetNamedPropertyValue(int index, ReadOnlySpan<byte> propertyName, out RdnElement value)
        {
            CheckNotDisposed();
        ...
        private bool TryGetNamedPropertyValue(
            ...
            out RdnElement value)
        {
            ReadOnlySpan<byte> documentSpan = _utf8Rdn.Span;
            Span<byte> utf8UnescapedStack = stackalloc byte[RdnConstants.StackallocByteThreshold];
            ...
            while (index > startIndex)
            {
                ...
                if (row.HasComplexChildren)
                {
                    ...
                    {
                        int idx = currentPropertyName.IndexOf(RdnConstants.BackSlash);
                        Debug.Assert(idx >= 0);
                            ...
                            currentPropertyName.Slice(0, idx).SequenceEqual(propertyName.Slice(0, idx)))
                        {
                            ...
                            try
                            {
                                ...
                                RdnReaderHelper.Unescape(currentPropertyName.Slice(idx), utf8Unescaped, 0, out written);
                                ...
                                if (utf8Unescaped.Slice(0, written).SequenceEqual(propertyName.Slice(idx)))
                                {
                                    ...
                                    value = new RdnElement(this, index + DbRow.Size);
                                    return true;
                                }
                            }
                            finally
