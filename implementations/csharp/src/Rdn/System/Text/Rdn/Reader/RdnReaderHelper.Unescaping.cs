...
namespace Rdn
{
    internal static partial class RdnReaderHelper
    {
        public static bool TryGetUnescapedBase64Bytes(ReadOnlySpan<byte> utf8Source, [NotNullWhen(true)] out byte[]? bytes)
        {
            ...
            Span<byte> utf8Unescaped = utf8Source.Length <= RdnConstants.StackallocByteThreshold ?
        ...
        public static string GetUnescapedString(ReadOnlySpan<byte> utf8Source)
        {
            ...
            Span<byte> utf8Unescaped = length <= RdnConstants.StackallocByteThreshold ?
                stackalloc byte[RdnConstants.StackallocByteThreshold] :
        ...
        {
            Debug.Assert(utf8Source.Length >= other.Length && utf8Source.Length / RdnConstants.MaxExpansionFactorWhileEscaping <= other.Length);
        ...
        public static bool UnescapeAndCompare(ReadOnlySequence<byte> utf8Source, ReadOnlySpan<byte> other)
        {
            ...
            Span<byte> utf8Escaped = length <= RdnConstants.StackallocByteThreshold ?
                stackalloc byte[RdnConstants.StackallocByteThreshold] :
        ...
        {
            int index1 = utf8Source1.IndexOf(RdnConstants.BackSlash);
            int index2 = utf8Source2.IndexOf(RdnConstants.BackSlash);
            ...
            Span<byte> utf8Unescaped1 = utf8Source1.Length <= RdnConstants.StackallocByteThreshold ?
                stackalloc byte[RdnConstants.StackallocByteThreshold] :
                (unescapedArray1 = ArrayPool<byte>.Shared.Rent(utf8Source1.Length));

            Span<byte> utf8Unescaped2 = utf8Source2.Length <= RdnConstants.StackallocByteThreshold ?
                stackalloc byte[RdnConstants.StackallocByteThreshold] :
        ...
        public static bool TryDecodeBase64(ReadOnlySpan<byte> utf8Unescaped, [NotNullWhen(true)] out byte[]? bytes)
        {
            ...
            Span<byte> byteSpan = utf8Unescaped.Length <= RdnConstants.StackallocByteThreshold ?
                stackalloc byte[RdnConstants.StackallocByteThreshold] :
        ...
        internal static void Unescape(ReadOnlySpan<byte> source, Span<byte> destination, out int written)
        {
            ...
            int idx = source.IndexOf(RdnConstants.BackSlash);
            Debug.Assert(idx >= 0);
        ...
        internal static void Unescape(ReadOnlySpan<byte> source, Span<byte> destination, int idx, out int written)
        {
            ...
            Debug.Assert(source[idx] == RdnConstants.BackSlash);
            Debug.Assert(destination.Length >= source.Length);
        ...
        private static bool TryUnescape(ReadOnlySpan<byte> source, Span<byte> destination, int idx, out int written)
        {
            ...
            while (true)
            {
                ...
                switch (source[++idx])
                {
                    case RdnConstants.Quote:
                        destination[written++] = RdnConstants.Quote;
                        break;
                    case (byte)'n':
                        destination[written++] = RdnConstants.LineFeed;
                        break;
                    case (byte)'r':
                        destination[written++] = RdnConstants.CarriageReturn;
                        break;
                    case RdnConstants.BackSlash:
                        destination[written++] = RdnConstants.BackSlash;
                        break;
                    case RdnConstants.Slash:
                        destination[written++] = RdnConstants.Slash;
                        break;
                    case (byte)'t':
                        destination[written++] = RdnConstants.Tab;
                        break;
                    case (byte)'b':
                        destination[written++] = RdnConstants.BackSpace;
                        break;
                    case (byte)'f':
                        destination[written++] = RdnConstants.FormFeed;
                        break;
                    default:
                        Debug.Assert(source[idx] == 'u', "invalid escape sequences must have already been caught by Utf8RdnReader.Read()");

                        // The source is known to be valid RDN, and hence if we see a \u, it is guaranteed to have 4 hex digits following it
                        // Otherwise, the Utf8RdnReader would have already thrown an exception.
                        ...
                        if (RdnHelpers.IsInRangeInclusive((uint)scalar, RdnConstants.HighSurrogateStartValue, RdnConstants.LowSurrogateEndValue))
                        {
                            // The first hex value cannot be a low surrogate.
                            if (scalar >= RdnConstants.LowSurrogateStartValue)
                            {
                                ThrowHelper.ThrowInvalidOperationException_ReadInvalidUTF16(scalar);
                            }

                            Debug.Assert(RdnHelpers.IsInRangeInclusive((uint)scalar, RdnConstants.HighSurrogateStartValue, RdnConstants.HighSurrogateEndValue));
                            ...
                            if (!RdnHelpers.IsInRangeInclusive((uint)lowSurrogate, RdnConstants.LowSurrogateStartValue, RdnConstants.LowSurrogateEndValue))
                            {
                            ...
                            scalar = (RdnConstants.BitShiftBy10 * (scalar - RdnConstants.HighSurrogateStartValue))
                                + (lowSurrogate - RdnConstants.LowSurrogateStartValue)
                                + RdnConstants.UnicodePlane01StartValue;
                        }
                ...
                if (source[idx] != RdnConstants.BackSlash)
                {
                    ReadOnlySpan<byte> remaining = source.Slice(idx);
                    int nextUnescapedSegmentLength = remaining.IndexOf(RdnConstants.BackSlash);
                    ...
                    Debug.Assert(idx == source.Length || source[idx] == RdnConstants.BackSlash);
