...
namespace Rdn.Serialization.Metadata
{
    public abstract partial class RdnTypeInfo
    {
        ...
        internal ReadOnlySpan<RdnParameterInfo> ParameterCache
        {
            get
            {
        ...
        private RdnParameterInfo[]? _parameterCache;

        // All of the serializable properties on a POCO (minus the extension property).
        internal ReadOnlySpan<RdnPropertyInfo> PropertyCache
        {
            get
            {
        ...
        private Dictionary<string, RdnPropertyInfo>? _propertyIndex;

        /// <summary>
        /// Stores a cache of UTF-8 encoded property names and their associated RdnPropertyInfo, if available.
        ...
        /// </summary>
        internal RdnPropertyInfo? GetProperty(ReadOnlySpan<byte> propertyName, ref ReadStackFrame frame, out byte[] utf8PropertyName)
        {
            Debug.Assert(IsConfigured);
            ...
            if (PropertyIndex.TryLookupUtf8Key(propertyName, out RdnPropertyInfo? info) &&
