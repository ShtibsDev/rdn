...
namespace Rdn.Serialization
{
    ...
    internal struct StreamReadBufferState : IReadBufferState<StreamReadBufferState, Stream>
    {
        ...
        // The counter is reset to zero whenever the RDN reader has been advanced successfully.
        //
        ...
        {
            _buffer = ArrayPool<byte>.Shared.Rent(Math.Max(initialBufferSize, RdnConstants.Utf8Bom.Length));
        ...
        private void ProcessReadBytes()
        {
            ...
            if (_isFirstBlock)
            {
                ...
                Debug.Assert(_buffer.Length >= RdnConstants.Utf8Bom.Length);
                if (_buffer.AsSpan(0, _count).StartsWith(RdnConstants.Utf8Bom))
                {
                    _offset = (byte)RdnConstants.Utf8Bom.Length;
                    _count -= RdnConstants.Utf8Bom.Length;
                }
            }
        }
