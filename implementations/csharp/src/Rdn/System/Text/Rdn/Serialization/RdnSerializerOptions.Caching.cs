...
namespace Rdn
{
    ...
    {
        /// <summary>
        /// Encapsulates all cached metadata referenced by the current <see cref="RdnSerializerOptions" /> instance.
        ...
        private volatile RdnTypeInfo? _lastTypeInfo;

        /// <summary>
        /// Gets the <see cref="RdnTypeInfo"/> contract metadata resolved by the current <see cref="RdnSerializerOptions"/> instance.
        /// </summary>
        ...
        /// <remarks>
        /// Returned metadata can be downcast to <see cref="RdnTypeInfo{T}"/> and used with the relevant <see cref="RdnSerializer"/> overloads.
        ///
        /// If the <see cref="RdnSerializerOptions"/> instance is locked for modification, the method will return a cached instance for the metadata.
        /// </remarks>
        public RdnTypeInfo GetTypeInfo(Type type)
        {
            ArgumentNullException.ThrowIfNull(type);

            if (RdnTypeInfo.IsInvalidForSerialization(type))
            {
        ...
        /// <summary>
        /// Tries to get the <see cref="RdnTypeInfo"/> contract metadata resolved by the current <see cref="RdnSerializerOptions"/> instance.
        /// </summary>
        /// <param name="type">The type to resolve contract metadata for.</param>
        ...
        /// If the <see cref="RdnSerializerOptions"/> instance is locked for modification, the method will return a cached instance for the metadata.
        /// </remarks>
        public bool TryGetTypeInfo(Type type, [NotNullWhen(true)] out RdnTypeInfo? typeInfo)
        {
            ArgumentNullException.ThrowIfNull(type);
        ...
        /// <summary>
        /// Gets the <see cref="RdnTypeInfo{T}"/> contract metadata resolved by the current <see cref="RdnSerializerOptions"/> instance.
        /// </summary>
        ...
        /// If the <see cref="RdnSerializerOptions"/> instance is locked for modification, the method will return a cached instance for the metadata.
        /// </remarks>
        public RdnTypeInfo<T> GetTypeInfo<T>()
        {
            return (RdnTypeInfo<T>)GetTypeInfo(typeof(T));
        }

        /// <summary>
        /// Tries to get the <see cref="RdnTypeInfo{T}"/> contract metadata resolved by the current <see cref="RdnSerializerOptions"/> instance.
        /// </summary>
        /// <typeparam name="T">The type to resolve contract metadata for.</typeparam>
        ...
        /// If the <see cref="RdnSerializerOptions"/> instance is locked for modification, the method will return a cached instance for the metadata.
        /// </remarks>
        public bool TryGetTypeInfo<T>([NotNullWhen(true)] out RdnTypeInfo<T>? typeInfo)
        {
            bool success = TryGetTypeInfo(typeof(T), out RdnTypeInfo? nonGeneric);
            typeInfo = (RdnTypeInfo<T>?)nonGeneric;
            return success;
        }
        ...
        internal RdnTypeInfo? GetTypeInfoInternal(
            Type type,
        ...
        internal bool TryGetTypeInfoCached(Type type, [NotNullWhen(true)] out RdnTypeInfo? typeInfo)
        {
        ...
        /// </summary>
        internal RdnTypeInfo GetTypeInfoForRootType(Type type, bool fallBackToNearestAncestorType = false)
        {
            RdnTypeInfo? rdnTypeInfo = _lastTypeInfo;

            if (rdnTypeInfo?.Type != type)
            {
                _lastTypeInfo = rdnTypeInfo = GetTypeInfoInternal(type, fallBackToNearestAncestorType: fallBackToNearestAncestorType);
            }

            return rdnTypeInfo;
        }

        internal bool TryGetPolymorphicTypeInfoForRootType(object rootValue, [NotNullWhen(true)] out RdnTypeInfo? polymorphicTypeInfo)
        {
        ...
        // Caches the resolved RdnTypeInfo<object> for faster access during root-level object type serialization.
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
        internal RdnTypeInfo ObjectTypeInfo
        {
            get
            {
                Debug.Assert(IsReadOnly);
                return _objectTypeInfo ??= GetTypeInfoInternal(RdnTypeInfo.ObjectType);
            }
        }

        private RdnTypeInfo? _objectTypeInfo;
        ...
        /// <summary>
        /// Stores and manages all reflection caches for one or more <see cref="RdnSerializerOptions"/> instances.
        ...
        internal sealed class CachingContext
        {
            ...
            public CachingContext(RdnSerializerOptions options, int hashCode)
            {
                Options = options;
                HashCode = hashCode;
#if !NET
            ...
            public RdnTypeInfo? GetOrAddTypeInfo(Type type, bool fallBackToNearestAncestorType = false)
            {
            ...
            private static CacheEntry CreateCacheEntry(Type type, CachingContext context)
            {
                try
                {
                    RdnTypeInfo? typeInfo = context.Options.GetTypeInfoNoCaching(type);
            ...
            private RdnTypeInfo? FallBackToNearestAncestor(Type type, CacheEntry entry)
            {
            ...
            private CacheEntry? DetermineNearestAncestor(Type type, CacheEntry entry)
            {
                ...
                {
                    if (current == RdnTypeInfo.ObjectType)
                    {
            ...
            private sealed class CacheEntry
            {
                ...
                public readonly RdnTypeInfo? TypeInfo;
                ...
                public CacheEntry(RdnTypeInfo? typeInfo)
                {
                    TypeInfo = typeInfo;
        ...
        internal static class TrackedCachingContexts
        {
            ...
            public static CachingContext GetOrCreate(RdnSerializerOptions options)
            {
                Debug.Assert(options.IsReadOnly, "Cannot create caching contexts for mutable RdnSerializerOptions instances");
        ...
        /// <summary>
        /// Provides a conservative equality comparison for RdnSerializerOptions instances.
        ...
        /// </summary>
        private sealed class EqualityComparer : IEqualityComparer<RdnSerializerOptions>
        {
            public bool Equals(RdnSerializerOptions? left, RdnSerializerOptions? right)
            {
            ...
            public int GetHashCode(RdnSerializerOptions options)
            {
                HashCode hc = default;
