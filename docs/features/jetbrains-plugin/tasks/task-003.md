# Task 003: Define Token Types

## References
- [Tech Design](../tech-design.md) — Section 5.2
- [Discovery](../discovery.md)

## Description
Create `RdnTokenTypes.kt` defining all ~52 `IElementType` token constants that the JFlex lexer will emit. This file is the shared vocabulary between the lexer, parser, and syntax highlighter. Every token type must be defined here before the lexer or highlighter can be implemented.

## Files to Create/Modify
- `tools/jetbrains-plugin/src/main/kotlin/com/rdn/intellij/lexer/RdnTokenTypes.kt` — All token type constants

## Implementation Details

Copy the full `RdnTokenTypes` object exactly as defined in Section 5.2 of the tech design:
...
**Note on KEY tokens:** The lexer itself does not emit `KEY_*` tokens directly. The base lexer emits `STRING_*` tokens for all strings. A wrapping `RdnHighlightingLexer` (implemented in task-006) looks ahead for `:` after `STRING_CLOSE` and re-maps those tokens to their `KEY_*` equivalents for syntax highlighting purposes. The `KEY_*` token types must exist in `RdnTokenTypes` for the highlighter to reference.

## Acceptance Criteria
- [ ] File compiles without errors
- [ ] All 52+ token type constants are defined (count them)
- [ ] Each token type has a unique debug name string
- [ ] `RdnTokenTypes.WHITE_SPACE` is `com.intellij.psi.TokenType.WHITE_SPACE` (not a new `RdnTokenType`)
- [ ] `RdnTokenTypes.BAD_CHARACTER` is `com.intellij.psi.TokenType.BAD_CHARACTER`
- [ ] `@JvmField` annotation is present on every field (required for Java interop with generated code)

## Dependencies
- Depends on: task-002
- Blocks: task-004, task-005, task-006, task-007, task-009
