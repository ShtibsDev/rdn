...
namespace Rdn
{
    internal static partial class RdnHelpers
    {
        ...
        private struct DateTimeParseData
        {
            ...
            public bool OffsetNegative => OffsetToken == RdnConstants.Hyphen;
        ...
        {
            return IsInRangeInclusive(length, RdnConstants.MinimumDateTimeParseLength, RdnConstants.MaximumEscapedDateTimeOffsetParseLength);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsValidUnescapedDateTimeOffsetParseLength(int length)
        {
            return IsInRangeInclusive(length, RdnConstants.MinimumDateTimeParseLength, RdnConstants.MaximumDateTimeOffsetParseLength);
        }
        ...
        public static bool TryParseAsISO(ReadOnlySpan<byte> source, out DateTime value)
        {
            ...
            if (parseData.OffsetToken == RdnConstants.UtcOffsetToken)
            {
                return TryCreateDateTime(parseData, DateTimeKind.Utc, out value);
            }
            else if (parseData.OffsetToken == RdnConstants.Plus || parseData.OffsetToken == RdnConstants.Hyphen)
            {
        ...
        public static bool TryParseAsISO(ReadOnlySpan<byte> source, out DateTimeOffset value)
        {
            ...
            if (parseData.OffsetToken == RdnConstants.UtcOffsetToken || // Same as specifying an offset of "+00:00", except that DateTime's Kind gets set to UTC rather than Local
                parseData.OffsetToken == RdnConstants.Plus || parseData.OffsetToken == RdnConstants.Hyphen)
            {
        ...
        private static bool TryParseDateTimeOffset(ReadOnlySpan<byte> source, out DateTimeParseData parseData)
        {
            ...
            if (source[10] != RdnConstants.TimePrefix || source[13] != RdnConstants.Colon
            ...
            switch (curByte)
            {
                case RdnConstants.UtcOffsetToken:
                    parseData.OffsetToken = RdnConstants.UtcOffsetToken;
                    return sourceIndex == source.Length;
                case RdnConstants.Plus:
                case RdnConstants.Hyphen:
                    parseData.OffsetToken = curByte;
                    return ParseOffset(ref parseData, source.Slice(sourceIndex));
                case RdnConstants.Colon:
                    break;
                default:
                    return false;
            }
            ...
            {
                int numDigitsRead = 0;
                int fractionEnd = Math.Min(sourceIndex + RdnConstants.DateTimeParseNumFractionDigits, source.Length);

                while (sourceIndex < fractionEnd && IsDigit(curByte = source[sourceIndex]))
                {
                    if (numDigitsRead < RdnConstants.DateTimeNumFractionDigits)
                    {
                ...
                {
                    while (numDigitsRead < RdnConstants.DateTimeNumFractionDigits)
                    {
            ...
            static bool ParseOffset(ref DateTimeParseData parseData, ReadOnlySpan<byte> offsetData)
            {
                ...
                if (offsetData.Length != 5
                    || offsetData[2] != RdnConstants.Colon
        ...
        {
            if (((uint)parseData.OffsetHours) > RdnConstants.MaxDateTimeUtcOffsetHours)
            {
                value = default;
                return false;
            }
            ...
            if (parseData.OffsetHours == RdnConstants.MaxDateTimeUtcOffsetHours && parseData.OffsetMinutes != 0)
            {
                value = default;
                return false;
            }
        ...
        private static bool TryCreateDateTime(DateTimeParseData parseData, DateTimeKind kind, out DateTime value)
        {
            ...
            Debug.Assert(parseData.Fraction >= 0 && parseData.Fraction <= RdnConstants.MaxDateTimeFraction); // All of our callers to date parse the fraction from fixed 7-digit fields so this value is trusted.
