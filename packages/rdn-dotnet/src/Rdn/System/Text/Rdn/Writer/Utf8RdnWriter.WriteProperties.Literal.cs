...
namespace Rdn
{
    ...
    {
        /// <summary>
        /// Writes the pre-encoded property name and the RDN literal "null" as part of a name/value pair of a RDN object.
        /// </summary>
        ...
        /// </exception>
        public void WriteNull(RdnEncodedText propertyName)
        {
            WriteLiteralHelper(propertyName.EncodedUtf8Bytes, RdnConstants.NullValue);
        ...
        internal void WriteNullSection(ReadOnlySpan<byte> escapedPropertyNameSection)
        {
            if (_options.Indented)
            {
                ...
                WriteLiteralHelper(escapedName, RdnConstants.NullValue);
                _tokenType = RdnTokenType.Null;
            }
            else
            {
                ...
                ReadOnlySpan<byte> span = RdnConstants.NullValue;
        ...
        /// <summary>
        /// Writes the property name and the RDN literal "null" as part of a name/value pair of a RDN object.
        /// </summary>
        ...
        /// <summary>
        /// Writes the pre-encoded property name and <see cref="bool"/> value (as a RDN literal "true" or "false") as part of a name/value pair of a RDN object.
        /// </summary>
        /// <param name="propertyName">The RDN-encoded name of the property to write.</param>
        ...
        /// Thrown if this would result in invalid RDN being written (while validation is enabled).
        /// </exception>
        public void WriteBoolean(RdnEncodedText propertyName, bool value)
        {
            if (value)
            {
                WriteLiteralHelper(propertyName.EncodedUtf8Bytes, RdnConstants.TrueValue);
                _tokenType = RdnTokenType.True;
            }
            else
            {
                WriteLiteralHelper(propertyName.EncodedUtf8Bytes, RdnConstants.FalseValue);
                _tokenType = RdnTokenType.False;
            }
        }

        /// <summary>
        /// Writes the property name and <see cref="bool"/> value (as a RDN literal "true" or "false") as part of a name/value pair of a RDN object.
        /// </summary>
        /// <param name="propertyName">The name of the property to write.</param>
        /// <param name="value">The value to write.</param>
        /// <exception cref="ArgumentException">
        ...
        public void WriteBoolean(ReadOnlySpan<char> propertyName, bool value)
        {
            ...
            ReadOnlySpan<byte> span = value ? RdnConstants.TrueValue : RdnConstants.FalseValue;

            WriteLiteralEscape(propertyName, span);

            SetFlagToAddListSeparatorBeforeNextItem();
            _tokenType = value ? RdnTokenType.True : RdnTokenType.False;
        }
        ...
        private void WriteLiteralMinimized(ReadOnlySpan<char> escapedPropertyName, ReadOnlySpan<byte> value)
        {
            ...
            Debug.Assert(escapedPropertyName.Length < (int.MaxValue / RdnConstants.MaxExpansionFactorWhileTranscoding) - value.Length - 4);

            // All ASCII, 2 quotes for property name, and 1 colon => escapedPropertyName.Length + value.Length + 3
            // Optionally, 1 list separator, and up to 3x growth when transcoding
            int maxRequired = (escapedPropertyName.Length * RdnConstants.MaxExpansionFactorWhileTranscoding) + value.Length + 4;
        ...
        private void WriteLiteralIndented(ReadOnlySpan<char> escapedPropertyName, ReadOnlySpan<byte> value)
        {
            ...
            Debug.Assert(value.Length <= RdnConstants.MaxUnescapedTokenSize);
            Debug.Assert(escapedPropertyName.Length < (int.MaxValue / RdnConstants.MaxExpansionFactorWhileTranscoding) - indent - value.Length - 5 - _newLineLength);

            // All ASCII, 2 quotes for property name, 1 colon, and 1 space => escapedPropertyName.Length + value.Length + 4
            // Optionally, 1 list separator, 1-2 bytes for new line, and up to 3x growth when transcoding
            int maxRequired = indent + (escapedPropertyName.Length * RdnConstants.MaxExpansionFactorWhileTranscoding) + value.Length + 5 + _newLineLength;
        ...
        {
            Span<byte> utf8PropertyName = stackalloc byte[RdnConstants.MaximumFormatBooleanLength];
