...
{
    /// <summary>
    /// Provides a high-performance API for forward-only, non-cached writing of UTF-8 encoded RDN text.
    /// </summary>
    /// <remarks>
    ///   <para>
    ///     It writes the text sequentially with no caching and adheres to the RDN RFC
    ///     by default (https://tools.ietf.org/html/rfc8259), with the exception of writing comments.
    ///   </para>
    ///   <para>
    ///     When the user attempts to write invalid RDN and validation is enabled, it throws
    ...
    ///     <see cref="RdnWriterOptions"/> and pass that in to the writer.
    ///   </para>
    /// </remarks>
    [DebuggerDisplay("{DebuggerDisplay,nq}")]
    public sealed partial class Utf8RdnWriter : IDisposable, IAsyncDisposable
    {
        ...
        private RdnWriterOptions _options; // Since RdnWriterOptions is a struct, use a field to avoid a copy for internal code.

        // Cache indentation settings from RdnWriterOptions to avoid recomputing them in the hot path.
        ...
        /// <summary>
        /// Returns the amount of bytes written by the <see cref="Utf8RdnWriter"/> so far
        /// that have not yet been flushed to the output and committed.
        /// </summary>
        public int BytesPending { get; private set; }

        /// <summary>
        /// Returns the amount of bytes committed to the output by the <see cref="Utf8RdnWriter"/> so far.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Gets the custom behavior when writing RDN using
        /// the <see cref="Utf8RdnWriter"/> which indicates whether to format the output
        /// while writing and whether to skip structural RDN validation or not.
        /// </summary>
        public RdnWriterOptions Options => _options;

        private int Indentation => CurrentDepth * _indentLength;

        internal RdnTokenType TokenType => _tokenType;

        /// <summary>
        /// Tracks the recursive depth of the nested objects / arrays within the RDN text
        /// written so far. This provides the depth of the current token.
        /// </summary>
        public int CurrentDepth => _currentDepth & RdnConstants.RemoveFlagsBitMask;
        ...
        private Utf8RdnWriter()
        {
        }

        /// <summary>
        /// Constructs a new <see cref="Utf8RdnWriter"/> instance with a specified <paramref name="bufferWriter"/>.
        /// </summary>
        /// <param name="bufferWriter">An instance of <see cref="IBufferWriter{Byte}" /> used as a destination for writing RDN text into.</param>
        /// <param name="options">Defines the customized behavior of the <see cref="Utf8RdnWriter"/>
        /// By default, the <see cref="Utf8RdnWriter"/> writes RDN minimized (that is, with no extra whitespace)
        /// and validates that the RDN being written is structurally valid according to RDN RFC.</param>
        /// <exception cref="ArgumentNullException">
        /// Thrown when the instance of <see cref="IBufferWriter{Byte}" /> that is passed in is null.
        /// </exception>
        public Utf8RdnWriter(IBufferWriter<byte> bufferWriter, RdnWriterOptions options = default)
        {
        ...
        /// <summary>
        /// Constructs a new <see cref="Utf8RdnWriter"/> instance with a specified <paramref name="utf8Rdn"/>.
        /// </summary>
        /// <param name="utf8Rdn">An instance of <see cref="Stream" /> used as a destination for writing RDN text into.</param>
        /// <param name="options">Defines the customized behavior of the <see cref="Utf8RdnWriter"/>
        /// By default, the <see cref="Utf8RdnWriter"/> writes RDN minimized (that is, with no extra whitespace)
        /// and validates that the RDN being written is structurally valid according to RDN RFC.</param>
        /// <exception cref="ArgumentNullException">
        ...
        /// </exception>
        public Utf8RdnWriter(Stream utf8Rdn, RdnWriterOptions options = default)
        {
        ...
        private void SetOptions(RdnWriterOptions options)
        {
            _options = options;
            ...
            {
                _options.MaxDepth = RdnWriterOptions.DefaultMaxDepth; // If max depth is not set, revert to the default depth.
            }
        }

        /// <summary>
        /// Resets the <see cref="Utf8RdnWriter"/> internal state so that it can be re-used.
        /// </summary>
        /// <remarks>
        /// The <see cref="Utf8RdnWriter"/> will continue to use the original writer options
        /// and the original output as the destination (either <see cref="IBufferWriter{Byte}" /> or <see cref="Stream" />).
        /// </remarks>
        /// <exception cref="ObjectDisposedException">
        ///   The instance of <see cref="Utf8RdnWriter"/> has been disposed.
        /// </exception>
        public void Reset()
        {
            CheckNotDisposed();
        ...
        /// <summary>
        /// Resets the <see cref="Utf8RdnWriter"/> internal state so that it can be re-used with the new instance of <see cref="Stream" />.
        /// </summary>
        /// <param name="utf8Rdn">An instance of <see cref="Stream" /> used as a destination for writing RDN text into.</param>
        /// <remarks>
        /// The <see cref="Utf8RdnWriter"/> will continue to use the original writer options
        ...
        ///   The instance of <see cref="Utf8RdnWriter"/> has been disposed.
        /// </exception>
        public void Reset(Stream utf8Rdn)
        {
            CheckNotDisposed();

            if (utf8Rdn == null)
                throw new ArgumentNullException(nameof(utf8Rdn));
            if (!utf8Rdn.CanWrite)
                throw new ArgumentException(SR.StreamNotWritable);
        ...
        /// <summary>
        /// Resets the <see cref="Utf8RdnWriter"/> internal state so that it can be re-used with the new instance of <see cref="IBufferWriter{Byte}" />.
        /// </summary>
        /// <param name="bufferWriter">An instance of <see cref="IBufferWriter{Byte}" /> used as a destination for writing RDN text into.</param>
        /// <remarks>
        /// The <see cref="Utf8RdnWriter"/> will continue to use the original writer options
        ...
        internal void Reset(IBufferWriter<byte> bufferWriter, RdnWriterOptions options)
        {
        ...
        internal static Utf8RdnWriter CreateEmptyInstanceForCaching() => new Utf8RdnWriter();
        ...
        private void CheckNotDisposed()
        {
            if (_stream == null)
            {
                ...
                if (_output == null)
                {
                    ThrowHelper.ThrowObjectDisposedException_Utf8RdnWriter();
                }
            }
        }

        /// <summary>
        /// Commits the RDN text written so far which makes it visible to the output destination.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        /// Commits any left over RDN text that has not yet been flushed and releases all resources used by the current instance.
        /// </summary>
        /// <remarks>
        ///   <para>
        ///     In the case of IBufferWriter, this advances the underlying <see cref="IBufferWriter{Byte}" /> based on what has been written so far.
        ...
        ///   </para>
        ///   <para>
        ///     The <see cref="Utf8RdnWriter"/> instance cannot be re-used after disposing.
        ///   </para>
        /// </remarks>
        public void Dispose()
        {
        ...
        /// <summary>
        /// Asynchronously commits any left over RDN text that has not yet been flushed and releases all resources used by the current instance.
        /// </summary>
        /// <remarks>
        ///   <para>
        ///     In the case of IBufferWriter, this advances the underlying <see cref="IBufferWriter{Byte}" /> based on what has been written so far.
        ...
        /// <summary>
        /// Asynchronously commits the RDN text written so far which makes it visible to the output destination.
        /// </summary>
        /// <remarks>
        /// In the case of IBufferWriter, this advances the underlying <see cref="IBufferWriter{Byte}" /> based on what has been written so far.
        ...
        /// <summary>
        /// Writes the beginning of a RDN array.
        /// </summary>
        /// <exception cref="InvalidOperationException">
        /// Thrown when the depth of the RDN has exceeded the maximum depth of 1000
        /// OR if this would result in invalid RDN being written (while validation is enabled).
        /// </exception>
        public void WriteStartArray()
        {
            WriteStart(RdnConstants.OpenBracket);
            _tokenType = RdnTokenType.StartArray;
        }

        /// <summary>
        /// Writes the beginning of a RDN object.
        /// </summary>
        /// <exception cref="InvalidOperationException">
        /// Thrown when the depth of the RDN has exceeded the maximum depth of 1000
        /// OR if this would result in invalid RDN being written (while validation is enabled).
        ...
        {
            WriteStart(RdnConstants.OpenBrace);
        ...
        private void OnValidateStartFailed()
        {
            ...
            {
                Debug.Assert(_tokenType != RdnTokenType.PropertyName);
                Debug.Assert(_tokenType != RdnTokenType.None && _tokenType != RdnTokenType.StartArray);
            ...
            else
            {
                ...
                Debug.Assert(CurrentDepth == 0 && _tokenType != RdnTokenType.None);
        ...
        /// <summary>
        /// Writes the beginning of a RDN array with a pre-encoded property name as the key.
        /// </summary>
        ...
        /// OR if this would result in invalid RDN being written (while validation is enabled).
        /// </exception>
        public void WriteStartArray(RdnEncodedText propertyName)
        {
            WriteStartHelper(propertyName.EncodedUtf8Bytes, RdnConstants.OpenBracket);
            _tokenType = RdnTokenType.StartArray;
        }

        /// <summary>
        /// Writes the beginning of a RDN object with a pre-encoded property name as the key.
        /// </summary>
        /// <param name="propertyName">The RDN-encoded name of the property to write.</param>
        /// <exception cref="InvalidOperationException">
        /// Thrown when the depth of the RDN has exceeded the maximum depth of 1000
        /// OR if this would result in invalid RDN being written (while validation is enabled).
        /// </exception>
        public void WriteStartObject(RdnEncodedText propertyName)
        {
            WriteStartHelper(propertyName.EncodedUtf8Bytes, RdnConstants.OpenBrace);
            _tokenType = RdnTokenType.StartObject;
        }
        ...
        /// <summary>
        /// Writes the beginning of a RDN array with a property name as the key.
        /// </summary>
        /// <param name="utf8PropertyName">The UTF-8 encoded property name of the RDN array to be written.</param>
        /// <remarks>
        ...
        public void WriteStartArray(ReadOnlySpan<byte> utf8PropertyName)
        {
            ...
            WriteStartEscape(utf8PropertyName, RdnConstants.OpenBracket);
        ...
        /// <summary>
        /// Writes the beginning of a RDN object with a property name as the key.
        /// </summary>
        ...
        public void WriteStartObject(ReadOnlySpan<byte> utf8PropertyName)
        {
            ...
            WriteStartEscape(utf8PropertyName, RdnConstants.OpenBrace);
        ...
        public void WriteStartArray(ReadOnlySpan<char> propertyName)
        {
            ...
            WriteStartEscape(propertyName, RdnConstants.OpenBracket);
        ...
        public void WriteStartObject(ReadOnlySpan<char> propertyName)
        {
            ...
            WriteStartEscape(propertyName, RdnConstants.OpenBrace);
        ...
        {
            WriteEnd(RdnConstants.CloseBracket);
            _tokenType = RdnTokenType.EndArray;
        }

        /// <summary>
        /// Writes the end of a RDN object.
        /// </summary>
        /// <exception cref="InvalidOperationException">
        /// Thrown if this would result in invalid RDN being written (while validation is enabled).
        /// </exception>
        ...
        private void ValidateEnd(byte token)
        {
            ...
            if (token == RdnConstants.CloseBracket)
            {
            ...
            else if (token == RdnConstants.CloseParen)
            {
            ...
            {
                Debug.Assert(token == RdnConstants.CloseBrace);
        ...
        {
            // Do not format/indent empty RDN object/array/set.
            if (_tokenType == RdnTokenType.StartObject || _tokenType == RdnTokenType.StartArray || _tokenType == RdnTokenType.StartSet || _tokenType == RdnTokenType.StartMap)
            {
                WriteEndMinimized(token);
            }
            else
            {
                ...
                Debug.Assert(indent <= _indentLength * _options.MaxDepth);
                Debug.Assert(_options.SkipValidation || _tokenType != RdnTokenType.None);
        ...
        private void WriteNewLine(Span<byte> output)
        {
            ...
            if (_newLineLength == 2)
            {
                output[BytesPending++] = RdnConstants.CarriageReturn;
            }
            output[BytesPending++] = RdnConstants.LineFeed;
        }

        private void WriteIndentation(Span<byte> buffer, int indent)
        {
            RdnWriterHelper.WriteIndentation(buffer, indent, _indentByte);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void UpdateBitStackOnStart(byte token)
        {
            if (token == RdnConstants.OpenBracket)
            {
                _bitStack.PushFalse();
            ...
            {
                Debug.Assert(token == RdnConstants.OpenBrace);
                _bitStack.PushTrue();
        ...
        private void Grow(int requiredSize)
        {
            ...
            if (_stream != null)
            {
                ...
                RdnHelpers.ValidateInt32MaxArrayLength((uint)needed);
        ...
        /// to allow <see cref="CanWriteValue"/> to be done using bitwise operations and must be kept in sync with <see cref="RdnTokenType"/>.
        /// </summary>
        internal enum EnclosingContainerType : byte
        {
            /// <summary>
            /// Root level. The choice of <see cref="RdnTokenType.None"/> allows fast validation by equality comparison when writing values
            /// since a value can be written at the root level only if there was no previous token.
            /// </summary>
            None = RdnTokenType.None,

            /// <summary>
            /// RDN object. The choice of <see cref="RdnTokenType.PropertyName"/> allows fast validation by equality comparison when writing values
            /// since a value can be written inside a RDN object only if the previous token is a property name.
            /// </summary>
            Object = RdnTokenType.PropertyName,

            /// <summary>
            /// RDN array. Chosen so that its lower nibble is 0 to ensure it does not conflict with <see cref="RdnTokenType"/> numeric values that currently are less than 16.
            /// </summary>
            Array = 0x10,
