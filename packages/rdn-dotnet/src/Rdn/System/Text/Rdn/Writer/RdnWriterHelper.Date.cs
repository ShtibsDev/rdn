...
namespace Rdn
{
    internal static partial class RdnWriterHelper
    {
        ...
        {
            Span<byte> tempSpan = stackalloc byte[RdnConstants.MaximumFormatDateTimeOffsetLength];
        ...
        {
            const int maxDateTimeLength = RdnConstants.MaximumFormatDateTimeLength;

            // Assert buffer is the right length for:
            // YYYY-MM-DDThh:mm:ss.fffffff (RdnConstants.MaximumFormatDateTimeLength)
            // YYYY-MM-DDThh:mm:ss.fffffffZ (RdnConstants.MaximumFormatDateTimeLength + 1)
            // YYYY-MM-DDThh:mm:ss.fffffff(+|-)hh:mm (RdnConstants.MaximumFormatDateTimeOffsetLength)
            Debug.Assert(buffer.Length == maxDateTimeLength ||
                buffer.Length == maxDateTimeLength + 1 ||
                buffer.Length == RdnConstants.MaximumFormatDateTimeOffsetLength);
            ...
            else if (buffer.Length == RdnConstants.MaximumFormatDateTimeOffsetLength)
            {
