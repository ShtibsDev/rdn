...
namespace Rdn
{
    internal static partial class RdnHelpers
    {
        /// <summary>
        /// Returns the unescaped span for the given reader.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ReadOnlySpan<byte> GetUnescapedSpan(this scoped ref Utf8RdnReader reader)
        {
            Debug.Assert(reader.TokenType is RdnTokenType.String or RdnTokenType.PropertyName);
            ReadOnlySpan<byte> span = reader.HasValueSequence ? reader.ValueSequence.ToArray() : reader.ValueSpan;
            return reader.ValueIsEscaped ? RdnReaderHelper.GetUnescaped(span) : span;
        }

        /// <summary>
        /// Attempts to perform a Read() operation and optionally checks that the full RDN value has been buffered.
        /// The reader will be reset if the operation fails.
        /// </summary>
        /// <param name="reader">The reader to advance.</param>
        /// <param name="requiresReadAhead">If reading a partial payload, read ahead to ensure that the full RDN value has been buffered.</param>
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool TryAdvanceWithOptionalReadAhead(this scoped ref Utf8RdnReader reader, bool requiresReadAhead)
        {
            // No read-ahead necessary if we're at the final block of RDN data.
            bool readAhead = requiresReadAhead && !reader.IsFinalBlock;
            return readAhead ? TryAdvanceWithReadAhead(ref reader) : reader.Read();
        }

        /// <summary>
        /// Attempts to read ahead to the next root-level RDN value, if it exists.
        /// </summary>
        public static bool TryAdvanceToNextRootLevelValueWithOptionalReadAhead(this scoped ref Utf8RdnReader reader, bool requiresReadAhead, out bool isAtEndOfStream)
        {
            ...
            Utf8RdnReader checkpoint = reader;
            if (!reader.Read())
            {
                // If the reader didn't return any tokens and it's the final block,
                // then there are no other RDN values to be read.
                isAtEndOfStream = reader.IsFinalBlock;
                reader = checkpoint;
                return false;
            }

            // We found another RDN value, read ahead accordingly.
            isAtEndOfStream = false;
            if (requiresReadAhead && !reader.IsFinalBlock)
            {
                // Perform full read-ahead to ensure the full RDN value has been buffered.
                reader = checkpoint;
        ...
        private static bool TryAdvanceWithReadAhead(scoped ref Utf8RdnReader reader)
        {
            ...
            if (tokenType is RdnTokenType.StartObject or RdnTokenType.StartArray)
            {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool IsInRangeInclusive(RdnTokenType value, RdnTokenType lowerBound, RdnTokenType upperBound)
            => (value - lowerBound) <= (upperBound - lowerBound);
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void ReadWithVerify(this ref Utf8RdnReader reader)
        {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void SkipWithVerify(this ref Utf8RdnReader reader)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool TrySkipPartial(this ref Utf8RdnReader reader)
        {
            ...
            [MaybeNullWhen(false)] out TValue result)
        {
            ...
            Span<char> charBuffer = utf8Key.Length <= RdnConstants.StackallocCharThreshold ?
        ...
        /// <summary>
        /// Compares two valid UTF-8 encoded RDN numbers for decimal equality.
        /// </summary>
        public static bool AreEqualRdnNumbers(ReadOnlySpan<byte> left, ReadOnlySpan<byte> right)
        {
            ...
            static void ParseNumber(
                ...
                out int exponent)
            {
                // Parses a RDN number into its integral, fractional, and exponent parts.
                ...
                {
                    Debug.Assert(char.IsDigit((char)span[0]), "leading plus not allowed in valid RDN numbers.");
                    neg = false;
                }
                ...
                if (!Utf8Parser.TryParse(span.Slice(i + 1), out exp, out _))
                {
                    ...
                    ThrowHelper.ThrowArgumentOutOfRangeException_RdnNumberExponentTooLarge(nameof(exponent));
                }
            ...
            Normalize: // Calculates the normal form of the number.
                ...
                if (intg[0] == '0')
                {
                    Debug.Assert(intg.Length == 1, "Leading zeros not permitted in RDN numbers.");
