...
namespace Rdn
{
    ...
    {
        // The database for the parsed structure of a RDN document.
        //
        ...
        private struct MetadataDb : IDisposable
        {
            ...
            // true      false  Used by RdnElement.ParseValue() for primitives and RdnDocument.Clone(). Size is known and no renting.
            // false     true   Used by RdnElement.ParseValue() for arrays and objects. Renting used until size is known.
            // true      true   not valid
            ...
            internal void Append(RdnTokenType tokenType, int startLocation, int length)
            {
                // StartArray or StartObject should have length -1, otherwise the length should not be -1.
                Debug.Assert(
                    (tokenType == RdnTokenType.StartArray || tokenType == RdnTokenType.StartObject || tokenType == RdnTokenType.StartSet || tokenType == RdnTokenType.StartMap) ==
            ...
            internal int FindIndexOfFirstUnsetSizeOrLength(RdnTokenType lookupType)
            {
                Debug.Assert(lookupType == RdnTokenType.StartObject || lookupType == RdnTokenType.StartArray || lookupType == RdnTokenType.StartSet || lookupType == RdnTokenType.StartMap);
                return FindOpenElement(lookupType);
            }

            private int FindOpenElement(RdnTokenType lookupType)
            {
            ...
            internal MetadataDb CopySegment(int startIndex, int endIndex)
            {
                ...
                if (start.TokenType == RdnTokenType.StartObject)
                {
                    Debug.Assert(
                        end.TokenType == RdnTokenType.EndObject,
                        $"StartObject paired with {end.TokenType}");
                }
                else if (start.TokenType == RdnTokenType.StartArray)
                {
                    Debug.Assert(
                        end.TokenType == RdnTokenType.EndArray,
                        $"StartArray paired with {end.TokenType}");
                }
                else if (start.TokenType == RdnTokenType.StartSet)
                {
                    Debug.Assert(
                        end.TokenType == RdnTokenType.EndSet,
                        $"StartSet paired with {end.TokenType}");
                }
                else if (start.TokenType == RdnTokenType.StartMap)
                {
                    Debug.Assert(
                        end.TokenType == RdnTokenType.EndMap,
                ...
                if (start.TokenType == RdnTokenType.String)
                {
                    locationOffset--;
                }
