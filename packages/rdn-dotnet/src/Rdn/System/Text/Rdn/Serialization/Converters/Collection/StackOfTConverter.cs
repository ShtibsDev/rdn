...
namespace Rdn.Serialization.Converters
{
        ...
        where TCollection : Stack<TElement>
    {
        ...
        protected override void CreateCollection(ref Utf8RdnReader reader, scoped ref ReadStack state, RdnSerializerOptions options)
        {
            ...
            if (state.Current.RdnTypeInfo.CreateObject == null)
            {
                ThrowHelper.ThrowNotSupportedException_SerializationNotSupported(state.Current.RdnTypeInfo.Type);
            }

            state.Current.ReturnValue = state.Current.RdnTypeInfo.CreateObject();
        }
    }
}
