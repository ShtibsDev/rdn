...
namespace Rdn
{
    public static partial class RdnSerializer
    {
        ...
        internal static bool TryReadMetadata(RdnConverter converter, RdnTypeInfo rdnTypeInfo, ref Utf8RdnReader reader, scoped ref ReadStack state)
        {
            Debug.Assert(state.Current.ObjectState == StackFrameObjectState.StartToken);
            Debug.Assert(state.Current.CanContainMetadata);

            Utf8RdnReader checkpoint;
            bool allowOutOfOrderMetadata = rdnTypeInfo.Options.AllowOutOfOrderMetadataProperties;
            ...
            while (true)
            {
                ...
                if (state.Current.PropertyState < StackFramePropertyState.Name)
                {
                    ...
                    if ((state.Current.MetadataPropertyNames & MetadataPropertyName.Ref) != 0)
                    {
                        ...
                        ThrowHelper.ThrowRdnException_MetadataReferenceObjectCannotContainOtherProperties(reader.GetUnescapedSpan(), ref state);
                    }

                    ReadOnlySpan<byte> propertyName = reader.GetUnescapedSpan();
                    switch (state.Current.LatestMetadataPropertyName = GetMetadataPropertyName(propertyName, rdnTypeInfo.PolymorphicTypeResolver))
                    {
                        case MetadataPropertyName.Id:
                            state.Current.RdnPropertyName = s_idPropertyName;

                            if (state.ReferenceResolver is null)
                            {
                                // Found an $id property in a type that doesn't support reference preservation
                                ThrowHelper.ThrowRdnException_MetadataUnexpectedProperty(propertyName, ref state);
                            }
                            if ((state.Current.MetadataPropertyNames & (MetadataPropertyName.Id | MetadataPropertyName.Ref)) != 0)
                            {
                                ...
                                ThrowHelper.ThrowRdnException_MetadataIdCannotBeCombinedWithRef(propertyName, ref state);
                            }
                            if (!converter.CanHaveMetadata)
                            {
                                // Should not be permitted unless the converter is capable of handling metadata.
                                ThrowHelper.ThrowRdnException_MetadataCannotParsePreservedObjectIntoImmutable(converter.Type!);
                            }

                            break;

                        case MetadataPropertyName.Ref:
                            state.Current.RdnPropertyName = s_refPropertyName;
                            ...
                            if (converter.IsValueType)
                            {
                                ...
                                ThrowHelper.ThrowRdnException_MetadataInvalidReferenceToValueType(converter.Type!);
                            }
                        ...
                        case MetadataPropertyName.Type:
                            state.Current.RdnPropertyName = rdnTypeInfo.PolymorphicTypeResolver?.CustomTypeDiscriminatorPropertyNameUtf8 ?? s_typePropertyName;

                            if (rdnTypeInfo.PolymorphicTypeResolver is null)
                            {
                            ...
                            if (state.PolymorphicTypeDiscriminator != null)
                            {
                                ...
                                ThrowHelper.ThrowRdnException_DuplicateMetadataProperty(state.Current.RdnPropertyName);
                            }

                            break;

                        case MetadataPropertyName.Values:
                            state.Current.RdnPropertyName = s_valuesPropertyName;

                            if ((state.Current.MetadataPropertyNames & MetadataPropertyName.Values) != 0)
                            {
                                // Found a duplicate $values property.
                                ThrowHelper.ThrowRdnException_DuplicateMetadataProperty(s_valuesPropertyName);
                            }

                            if (isReadingAheadOfNonMetadataProperties)
                            {
                                // Cannot combine a $values property with other non-metadata properties.
                                ThrowHelper.ThrowRdnException_MetadataInvalidPropertyInArrayMetadata(ref state, rdnTypeInfo.Type, reader);
                            }
                ...
                switch (state.Current.LatestMetadataPropertyName)
                {
                    case MetadataPropertyName.Id:
                        ...
                        {
                            ThrowHelper.ThrowRdnException_MetadataValueWasNotString(reader.TokenType);
                        }
                    ...
                    case MetadataPropertyName.Values:
                        ...
                        {
                            ThrowHelper.ThrowRdnException_MetadataValuesInvalidToken(reader.TokenType);
                        }
        ...
        Done:
            if (state.Current.MetadataPropertyNames is MetadataPropertyName.Values)
            {
                ...
                ThrowHelper.ThrowRdnException_MetadataStandaloneValuesProperty(ref state, s_valuesPropertyName);
            }
        ...
        internal static bool TryHandleReferenceFromRdnElement(
            ...
            [NotNullWhen(true)] out object? referenceValue)
        {
            ...
            if (element.ValueKind == RdnValueKind.Object)
            {
                int propertyCount = 0;
                foreach (RdnProperty property in element.EnumerateObject())
                {
                    propertyCount++;
                    if (refMetadataFound)
                    {
                        // There are more properties in an object with $ref.
                        ThrowHelper.ThrowRdnException_MetadataReferenceObjectCannotContainOtherProperties();
                    }
                    else if (property.EscapedNameEquals(s_idPropertyName))
                    {
                        ...
                        if (property.Value.ValueKind != RdnValueKind.String)
                        {
                            ThrowHelper.ThrowRdnException_MetadataValueWasNotString(property.Value.ValueKind);
                        }
        ...
        internal static bool TryHandleReferenceFromRdnNode(
            ref Utf8RdnReader reader,
            scoped ref ReadStack state,
            RdnNode? rdnNode,
            [NotNullWhen(true)] out object? referenceValue)
        {
            ...
            if (rdnNode is RdnObject rdnObject)
            {
                int propertyCount = 0;
                foreach (KeyValuePair<string, RdnNode?> property in rdnObject)
                {
                    propertyCount++;
                    if (refMetadataFound)
                    {
                    ...
                    else if (property.Key == "$id")
                    {
                        ...
                        state.ReferenceResolver.AddReference(referenceId, rdnNode);
                        referenceValue = rdnNode;
                        return true;
                    }
                    ...
                    static string ReadAsStringMetadataValue(RdnNode? rdnNode)
                    {
                        if (rdnNode is RdnValue rdnValue &&
                            rdnValue.TryGetValue(out string? value) &&
                            value is not null)
                        {
                            return value;
                        }

                        RdnValueKind metadataValueKind = rdnNode?.GetValueKind() ?? RdnValueKind.Null;
                        Debug.Assert(metadataValueKind != RdnValueKind.Undefined);
                        ThrowHelper.ThrowRdnException_MetadataValueWasNotString(metadataValueKind);
                        return null!;
                    }
                }
            }
        ...
        internal static void ValidateMetadataForObjectConverter(ref ReadStack state)
        {
            if ((state.Current.MetadataPropertyNames & MetadataPropertyName.Values) != 0)
            {
                ...
                ThrowHelper.ThrowRdnException_MetadataUnexpectedProperty(s_valuesPropertyName, ref state);
            }
        }

        internal static void ValidateMetadataForArrayConverter(RdnConverter converter, ref Utf8RdnReader reader, scoped ref ReadStack state)
        {
            switch (reader.TokenType)
            {
                ...
                case RdnTokenType.EndObject:
                    ...
                    {
                        // Read the entire RDN object while parsing for metadata: for collection converters this is only legal for $ref nodes.
                        ThrowHelper.ThrowRdnException_MetadataPreservedArrayValuesNotFound(ref state, converter.Type!);
                    }
                    break;
                ...
                default:
                    ...
                    ThrowHelper.ThrowRdnException_MetadataInvalidPropertyInArrayMetadata(ref state, converter.Type!, reader);
                    break;
            }
        }
