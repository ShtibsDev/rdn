...
namespace Rdn.Serialization.Metadata
{
    ...
    /// </summary>
    internal sealed class RdnPropertyInfo<T> : RdnPropertyInfo
    {
        private Func<object, T>? _typedGet;
        private Action<object, T>? _typedSet;

        internal RdnPropertyInfo(Type declaringType, RdnTypeInfo? declaringTypeInfo, RdnSerializerOptions options)
        ...
        internal override void AddRdnParameterInfo(RdnParameterInfoValues parameterInfoValues)
        {
            Debug.Assert(!IsConfigured);
            Debug.Assert(AssociatedParameter is null);

            AssociatedParameter = new RdnParameterInfo<T>(parameterInfoValues, this);
        ...
        internal new RdnConverter<T> EffectiveConverter
        {
            get
            {
        ...
        private RdnConverter<T>? _typedEffectiveConverter;
        ...
        internal override void DetermineReflectionPropertyAccessors(MemberInfo memberInfo, bool useNonPublicAccessors)
            => DefaultRdnTypeInfoResolver.DeterminePropertyAccessors<T>(this, memberInfo, useNonPublicAccessors);

        private protected override void DetermineEffectiveConverter(RdnTypeInfo rdnTypeInfo)
        {
            Debug.Assert(rdnTypeInfo is RdnTypeInfo<T>);
            ...
            RdnConverter<T> converter =
                ...
                ?.CreateCastingConverter<T>()                                 // Cast to RdnConverter<T>, potentially with wrapping.
                ?? ((RdnTypeInfo<T>)rdnTypeInfo).EffectiveConverter;        // Fall back to the effective converter for the type.
        ...
        internal override bool GetMemberAndWriteRdn(object obj, ref WriteStack state, Utf8RdnWriter writer)
        {
            T value = Get!(obj);
            ...
            if (value is null)
            {
                ...
                {
                    ThrowHelper.ThrowRdnException_PropertyGetterDisallowNull(Name, state.Current.RdnTypeInfo.Type);
                }
                ...
                if (EffectiveConverter.HandleNullOnWrite)
                {
                    ...
                    {
                        ThrowHelper.ThrowRdnException_SerializationConverterWrite(EffectiveConverter);
                    }
                }
                else
                {
        ...
        internal override bool GetMemberAndWriteRdnExtensionData(object obj, ref WriteStack state, Utf8RdnWriter writer)
        {
            bool success;
            T value = Get!(obj);
        ...
        internal override bool ReadRdnAndSetMember(object obj, scoped ref ReadStack state, ref Utf8RdnReader reader)
        {
            bool success;

            bool isNullToken = reader.TokenType == RdnTokenType.Null;
            ...
            if (isNullToken && !EffectiveConverter.HandleNullOnRead && !state.IsContinuation)
            {
                if (default(T) is not null || !CanDeserialize)
                {
                    ...
                    {
                        Debug.Assert(CanDeserialize || EffectiveObjectCreationHandling == RdnObjectCreationHandling.Populate);
                        ThrowHelper.ThrowInvalidOperationException_DeserializeUnableToAssignNull(EffectiveConverter.Type);
                    }

                    ThrowHelper.ThrowRdnException_DeserializeUnableToConvertValue(EffectiveConverter.Type);
                }
                ...
                if (!IgnoreNullTokensOnRead)
                {
                    ...
                    {
                        ThrowHelper.ThrowRdnException_PropertySetterDisallowNull(Name, state.Current.RdnTypeInfo.Type);
                    }
            ...
            else if (EffectiveConverter.CanUseDirectReadOrWrite && state.Current.NumberHandling == null)
            {
                ...
                Debug.Assert(EffectiveObjectCreationHandling != RdnObjectCreationHandling.Populate, "Populating should not be possible for simple types");
        ...
        internal override bool ReadRdnAsObject(scoped ref ReadStack state, ref Utf8RdnReader reader, out object? value)
        {
            bool success;
            bool isNullToken = reader.TokenType == RdnTokenType.Null;
            if (isNullToken && !EffectiveConverter.HandleNullOnRead && !state.IsContinuation)
        ...
        private protected override void ConfigureIgnoreCondition(RdnIgnoreCondition? ignoreCondition)
        {
            switch (ignoreCondition)
            {
                ...
                case RdnIgnoreCondition.WhenWritingDefault:
                    ShouldSerialize = ShouldSerializeIgnoreWhenWritingDefault;
                    IgnoreDefaultValuesOnWrite = true;
                    break;

                case RdnIgnoreCondition.WhenWriting:
                    ShouldSerialize = ShouldSerializeIgnoreConditionAlways;
                    break;

                case RdnIgnoreCondition.WhenReading:
                    Set = null;
                    break;

                default:
                    Debug.Fail($"Unknown value of RdnIgnoreCondition '{ignoreCondition}'");
                    break;
            }
