...
{
    /// <summary>
    /// Represents a UTF-8 encoded RDN property name and its associated <see cref="RdnPropertyInfo"/>, if available.
    /// PropertyRefs use byte sequence equality, so equal RDN strings with alternate encodings or casings are not equal.
    /// Used as a first-level cache for property lookups before falling back to UTF decoding and string comparison.
    /// </summary>
    internal readonly struct PropertyRef(ulong key, RdnPropertyInfo? info, byte[] utf8PropertyName) : IEquatable<PropertyRef>
    {
        ...
        /// <summary>
        /// The <see cref="RdnPropertyInfo"/> associated with the property name, if available.
        /// </summary>
        public readonly RdnPropertyInfo? Info = info;
