...
{
    /// <summary>
    /// Provides metadata about a set of types that is relevant to RDN serialization.
    /// </summary>
    public abstract partial class RdnSerializerContext : IRdnTypeInfoResolver, IBuiltInRdnTypeInfoResolver
    {
        ...
        /// </remarks>
        public RdnSerializerOptions Options
        {
            get
            {
                RdnSerializerOptions? options = _options;

                if (options is null)
                {
                    options = new RdnSerializerOptions { TypeInfoResolver = this };
        ...
        internal void AssociateWithOptions(RdnSerializerOptions options)
        {
        ...
        /// is compatible with the run time specified <see cref="RdnSerializerOptions"/>.
        /// </summary>
        bool IBuiltInRdnTypeInfoResolver.IsCompatibleWithOptions(RdnSerializerOptions options)
        {
            Debug.Assert(options != null);

            RdnSerializerOptions? generatedSerializerOptions = GeneratedSerializerOptions;
            ...
            return
                ...
                !RdnHelpers.RequiresSpecialNumberHandlingOnWrite(options.NumberHandling) &&
                options.ReferenceHandlingStrategy == RdnKnownReferenceHandler.Unspecified &&
...
#pragma warning restore SYSLIB0020
        ...
        /// <summary>
        /// The default run time options for the context. Its values are defined at design-time via <see cref="RdnSourceGenerationOptionsAttribute"/>.
        /// </summary>
        protected abstract RdnSerializerOptions? GeneratedSerializerOptions { get; }

        /// <summary>
        /// Creates an instance of <see cref="RdnSerializerContext"/> and binds it with the indicated <see cref="RdnSerializerOptions"/>.
        /// </summary>
        /// <param name="options">The run time provided options for the context instance.</param>
        /// <remarks>
        /// If no instance options are passed, then no options are set until the context is bound using <see cref="RdnSerializerOptions.AddContext{TContext}"/>,
        /// or until <see cref="Options"/> is called, where a new options instance is created and bound.
        /// </remarks>
        protected RdnSerializerContext(RdnSerializerOptions? options)
        {
            if (options != null)
            {
        ...
        /// <summary>
        /// Returns a <see cref="RdnTypeInfo"/> instance representing the given type.
        /// </summary>
        ...
        public abstract RdnTypeInfo? GetTypeInfo(Type type);

        RdnTypeInfo? IRdnTypeInfoResolver.GetTypeInfo(Type type, RdnSerializerOptions options)
        {
