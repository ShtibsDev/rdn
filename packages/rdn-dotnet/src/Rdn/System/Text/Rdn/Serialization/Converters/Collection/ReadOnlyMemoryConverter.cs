...
{
    internal sealed class ReadOnlyMemoryConverter<T> : RdnCollectionConverter<ReadOnlyMemory<T>, T>
    {
        ...
        protected override bool OnWriteResume(Utf8RdnWriter writer, ReadOnlyMemory<T> value, RdnSerializerOptions options, ref WriteStack state)
        {
            return OnWriteResume(writer, value.Span, options, ref state);
        }

        internal static bool OnWriteResume(Utf8RdnWriter writer, ReadOnlySpan<T> value, RdnSerializerOptions options, ref WriteStack state)
        {
            int index = state.Current.EnumeratorIndex;

            RdnConverter<T> elementConverter = GetElementConverter(ref state);
