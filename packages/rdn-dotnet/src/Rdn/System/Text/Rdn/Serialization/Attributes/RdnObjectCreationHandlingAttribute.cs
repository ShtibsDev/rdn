...
/// When default resolvers are used this will be mapped to <see cref="RdnPropertyInfo.ObjectCreationHandling"/>.
///
/// When placed on a type with <see cref="RdnObjectCreationHandling.Populate"/> indicates that all members
/// that support population will be populated. When default resolvers are used this will be mapped to <see cref="RdnTypeInfo.PreferredPropertyObjectCreationHandling"/>.
///
...
/// For example when <see cref="RdnObjectCreationHandlingAttribute"/> with <see cref="RdnObjectCreationHandling.Populate"/> is placed on a class
...
public sealed class RdnObjectCreationHandlingAttribute : RdnAttribute
{
    /// <summary>
    /// Indicates what configuration should be used when deserializing members.
    /// </summary>
    public RdnObjectCreationHandling Handling { get; }

    /// <summary>
    /// Initializes a new instance of <see cref="RdnObjectCreationHandlingAttribute"/>.
    /// </summary>
    /// <param name="handling">The handling to apply to the current member.</param>
    public RdnObjectCreationHandlingAttribute(RdnObjectCreationHandling handling)
    {
        if (!RdnSerializer.IsValidCreationHandlingValue(handling))
        {
