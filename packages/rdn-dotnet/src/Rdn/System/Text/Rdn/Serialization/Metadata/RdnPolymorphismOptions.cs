...
namespace Rdn.Serialization.Metadata
{
    ...
    /// </summary>
    public class RdnPolymorphismOptions
    {
        private DerivedTypeList? _derivedTypes;
        private bool _ignoreUnrecognizedTypeDiscriminators;
        private RdnUnknownDerivedTypeHandling _unknownDerivedTypeHandling;
        private string? _typeDiscriminatorPropertyName;

        /// <summary>
        /// Creates an empty <see cref="RdnPolymorphismOptions"/> instance.
        /// </summary>
        public RdnPolymorphismOptions()
        {
        }
        ...
        /// </summary>
        public IList<RdnDerivedType> DerivedTypes => _derivedTypes ??= new(this);
        ...
        /// The parent <see cref="RdnTypeInfo"/> instance has been locked for further modification.
        /// </exception>
        ...
        /// The parent <see cref="RdnTypeInfo"/> instance has been locked for further modification.
        /// </exception>
        public RdnUnknownDerivedTypeHandling UnknownDerivedTypeHandling
        {
        ...
        {
            get => _typeDiscriminatorPropertyName ?? RdnSerializer.TypePropertyName;
            set
            {
                VerifyMutable();
        ...
        internal RdnTypeInfo? DeclaringTypeInfo { get; set; }

        private sealed class DerivedTypeList : ConfigurationList<RdnDerivedType>
        {
            private readonly RdnPolymorphismOptions _parent;

            public DerivedTypeList(RdnPolymorphismOptions parent)
            {
                _parent = parent;
            }
        ...
        internal static RdnPolymorphismOptions? CreateFromAttributeDeclarations(Type baseType)
        {
            RdnPolymorphismOptions? options = null;

            if (baseType.GetCustomAttribute<RdnPolymorphicAttribute>(inherit: false) is RdnPolymorphicAttribute polymorphicAttribute)
            {
                options = new()
                {
            ...
            foreach (RdnDerivedTypeAttribute attr in baseType.GetCustomAttributes<RdnDerivedTypeAttribute>(inherit: false))
            {
                (options ??= new()).DerivedTypes.Add(new RdnDerivedType(attr.DerivedType, attr.TypeDiscriminator));
            }
