    ...
    public partial class RdnTypeInfo<T>
    {
            ...
            object? rootValueBoxed = null)
        {
            ...
            if (CanUseSerializeHandler)
            {
                ...
                // Even though this is already handled by RdnMetadataServicesConverter,
                ...
                Options.TryGetPolymorphicTypeInfoForRootType(rootValue, out RdnTypeInfo? derivedTypeInfo))
            {
        ...
        internal Task SerializeAsync(Stream utf8Rdn,
            T? rootValue,
            CancellationToken cancellationToken,
            object? rootValueBoxed = null)
        {
            PooledByteBufferWriter writer = new PooledByteBufferWriter(Options.DefaultBufferSize, utf8Rdn);
            ...
            int flushThreshold = (int)(writer.Capacity * RdnSerializer.FlushThreshold);
            return SerializeAsync(writer, rootValue, flushThreshold, cancellationToken, rootValueBoxed);
        }

        internal Task SerializeAsync(PipeWriter utf8Rdn,
            T? rootValue,
            CancellationToken cancellationToken,
            object? rootValueBoxed = null)
        {
            // Value chosen as 90% of 4 buffer segments in Pipes. This is semi-arbitrarily chosen and may be changed in future iterations.
            int flushThreshold = (int)(4 * PipeOptions.Default.MinimumSegmentSize * RdnSerializer.FlushThreshold);
            return SerializeAsync(utf8Rdn, rootValue, flushThreshold, cancellationToken, rootValueBoxed);
        }
            ...
            object? rootValueBoxed = null)
        {
            ...
            else
            {
                ...
                var writer = new Utf8RdnWriter(pipeWriter, Options.GetWriterOptions());
            ...
            object? rootValueBoxed = null)
        {
            ...
            if (CanUseSerializeHandlerInStreaming)
            {
                ...
                try
                {
                    SerializeHandler(writer, rootValue!);
                    writer.Flush();
                    bufferWriter.WriteToStream(utf8Rdn);
                }
                finally
                {
                    // Record the serialization size in both successful and failed operations,
                    ...
                    Utf8RdnWriterCache.ReturnWriterAndBuffer(writer, bufferWriter);
                }
            }
            else if (
#if NET
                ...
                Converter.CanBePolymorphic &&
                rootValue is not null &&
                Options.TryGetPolymorphicTypeInfoForRootType(rootValue, out RdnTypeInfo? polymorphicTypeInfo))
            {
                Debug.Assert(typeof(T) == typeof(object));
                polymorphicTypeInfo.SerializeAsObject(utf8Rdn, rootValue);
            }
            else
            {
                bool isFinalBlock;
                ...
                try
                {
                    ...
                    state.FlushThreshold = (int)(bufferWriter.Capacity * RdnSerializer.FlushThreshold);
        ...
        internal sealed override void SerializeAsObject(Utf8RdnWriter writer, object? rootValue)
            => Serialize(writer, RdnSerializer.UnboxOnWrite<T>(rootValue), rootValue);
        ...
        internal sealed override Task SerializeAsObjectAsync(PipeWriter pipeWriter, object? rootValue, int flushThreshold, CancellationToken cancellationToken)
            => SerializeAsync(pipeWriter, RdnSerializer.UnboxOnWrite<T>(rootValue), flushThreshold, cancellationToken, rootValue);

        internal sealed override Task SerializeAsObjectAsync(Stream utf8Rdn, object? rootValue, CancellationToken cancellationToken)
            => SerializeAsync(utf8Rdn, RdnSerializer.UnboxOnWrite<T>(rootValue), cancellationToken, rootValue);

        internal sealed override Task SerializeAsObjectAsync(PipeWriter utf8Rdn, object? rootValue, CancellationToken cancellationToken)
            => SerializeAsync(utf8Rdn, RdnSerializer.UnboxOnWrite<T>(rootValue), cancellationToken, rootValue);

        internal sealed override void SerializeAsObject(Stream utf8Rdn, object? rootValue)
            => Serialize(utf8Rdn, RdnSerializer.UnboxOnWrite<T>(rootValue), rootValue);
        ...
        // assuming that the current type is known to produce small enough RDN payloads.
        ...
        //  * No serialization size exceeding RdnSerializerOptions.DefaultBufferSize / 2 has been recorded so far.
