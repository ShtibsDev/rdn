...
{
    internal sealed class TimeOnlyConverter : RdnPrimitiveConverter<TimeOnly>
    {
        private const int MinimumTimeOnlyFormatLength = 3; // h:m
        private const int MaximumTimeOnlyFormatLength = 16; // hh:mm:ss.fffffff
        private const int MaximumEscapedTimeOnlyFormatLength = RdnConstants.MaxExpansionFactorWhileEscaping * MaximumTimeOnlyFormatLength;

        public override TimeOnly Read(ref Utf8RdnReader reader, Type typeToConvert, RdnSerializerOptions options)
        {
            if (reader.TokenType == RdnTokenType.RdnTimeOnly)
            {
        ...
        internal override TimeOnly ReadAsPropertyNameCore(ref Utf8RdnReader reader, Type typeToConvert, RdnSerializerOptions options)
        {
            Debug.Assert(reader.TokenType == RdnTokenType.PropertyName);
            return ReadCore(ref reader);
        }

        private static TimeOnly ReadCore(ref Utf8RdnReader reader)
        {
            Debug.Assert(reader.TokenType is RdnTokenType.String or RdnTokenType.PropertyName);

            if (!RdnHelpers.IsInRangeInclusive(reader.ValueLength, MinimumTimeOnlyFormatLength, MaximumEscapedTimeOnlyFormatLength))
            {
            ...
            if (!RdnHelpers.IsDigit(firstChar) || firstSeparator < 0 || source[firstSeparator] == (byte)'.')
            {
        ...
        public override void Write(Utf8RdnWriter writer, TimeOnly value, RdnSerializerOptions options)
        {
            writer.WriteRdnTimeOnlyValue(value);
        }

        internal override void WriteAsPropertyNameCore(Utf8RdnWriter writer, TimeOnly value, RdnSerializerOptions options, bool isWritingExtensionDataProperty)
        {
